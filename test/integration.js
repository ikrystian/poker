// Test end-to-end: 4 klienty łączą się z serwerem i rozgrywają rozdanie.
import { io } from 'socket.io-client';

const URL = process.env.URL || 'http://localhost:3777';
const TABLE = 'test' + Math.floor(Math.random() * 100000);
const NAMES = ['Ala', 'Bartek', 'Celina', 'Daniel'];
...
// Piąty gracz nie powinien się zmieścić.
const extra = io(URL, { transports: ['websocket'] });
await new Promise((r) => extra.on('connect', r));
const full = await emit(extra, 'join', { tableId: TABLE, name: 'Ewa' });
check(!!full.error, 'piąty gracz powinien zostać odrzucony');
extra.close();
...
check(s0.players.length === 4, 'przy stole powinno być 4 graczy');
...
check(
  end.players.reduce((s, p) => s + p.chips, 0) === 4000,
  'suma żetonów po rozdaniu powinna wynosić 4000',
);
check(
...
const pid = clients[3].pid;
clients[3].close();
await wait(200);
const back = io(URL, { transports: ['websocket'] });
await new Promise((r) => back.on('connect', r));
const rejoin = await emit(back, 'join', { tableId: TABLE, name: 'Daniel', playerId: pid });
